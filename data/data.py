recipes = [
    {
        "id": 1,
        "name": "肉じゃが",
        "category": "main",
        "ingredients": ["じゃがいも", "玉ねぎ", "牛肉", "にんじん","白滝"],
        "steps": ["1. 具材を切る。", "2. 炒める。", "3. 煮詰める。"],
        "genre": "japanese"
    },
    {
        "id": 2,
        "name": "麻婆茄子",
        "category": "main",
        "ingredients": ["なす", "豚ひき肉", "長ねぎ"],
        "steps": ["1. 具材を切る。", "2. 調味料にからめる。", "3. 炒める。", "4. 茄子以外を煮る。", "5. 茄子を混ぜて加熱する。"], 
        "genre": "japanese"
    },
    {
        "id": 3,
        "name": "しょうが焼き",
        "category": "main",
        "ingredients": ["豚ロース", "玉ねぎ"],
        "steps": ["1. 豚肉に下味を付ける。", "2. 玉ねぎを切る。", "3. 炒める。", "4. 調味料を加えて煮からめる。"],
        "genre": "japanese"
    },
    {
        "id": 4,
        "name": "ロールキャベツ",
        "category": "main",
        "ingredients": ["キャベツ", "合いびき肉", "玉ねぎ", "パン粉", "牛乳", "卵"],
        "steps": ["1. 具材を切る。", "2. 肉だねを作る。", "3. キャベツで包む。", "4. 煮込む。"],
        "genre": "western"
    },
    {
        "id": 5,
        "name": "ハンバーグ",
        "category": "main",
        "ingredients": ["合いびき肉", "玉ねぎ", "パン粉"],
        "steps": ["1. 玉ねぎを炒める。", "2. 肉だねを作る。", "3. 成形する。", "4. 焼く。"],
        "genre": "western"
    },
    {
        "id": 6,
        "name": "ピーマンの肉詰め",
        "category": "main",
        "ingredients": ["ピーマン", "合いびき肉", "玉ねぎ"],
        "steps": ["1. ピーマンを切る。", "2. 肉だねを作る。", "3. ピーマンに詰める。", "4. 焼く。"],
        "genre": "western"
    },
    {
        "id": 7,
        "name": "鶏の照り焼き",
        "category": "main",
        "ingredients": ["鶏もも肉"],
        "steps": ["1. 鶏肉に下味を付ける。", "2. 焼く。", "3. 調味料を加えて煮からめる。"],
        "genre": "japanese"
    },
    {
        "id": 8,
        "name": "唐揚げ",
        "category": "main",
        "ingredients": ["鶏もも肉"],
        "steps": ["1. 鶏肉に下味を付ける。", "2. 衣を付ける。", "3. 揚げる。"],
        "genre": "japanese"
    },
    {
        "id": 9,
        "name": "とんかつ",
        "category": "main",
        "ingredients": ["豚ロース", "卵", "パン粉"],
        "steps": ["1. 豚肉に下味を付ける。", "2. 衣を付ける。", "3. 揚げる。"],
        "genre": "japanese"
    },
    {
        "id": 10,
        "name": "チキン南蛮",
        "category": "main",
        "ingredients": ["鶏むね肉", "卵"],
        "steps": ["1. 鶏肉に下味を付ける。", "2. 衣を付けて揚げる。", "3. 甘酢にくぐらせる。", "4. タルタルソースをかける。"],
        "genre": "japanese"
    },
    {
        "id": 11,
        "name": "ぶりの照り焼き",
        "category": "main",
        "ingredients": ["ぶり"],
        "steps": ["1. ぶりに下味を付ける。", "2. 焼く。", "3. 調味料を加えて煮からめる。"], 
        "genre": "japanese"
    },
    {
        "id": 12,
        "name": "鮭のムニエル",
        "category": "main",
        "ingredients": ["鮭", "バター"],
        "steps": ["1. 鮭に下味を付ける。", "2. 小麦粉をまぶす。", "3. 焼く。", "4. バターを加える。"],
        "genre": "western"
    },
    {
        "id": 13,
        "name": "さばの味噌煮",
        "category": "main",
        "ingredients": ["さば", "味噌", "しょうが"],
        "steps": ["1. さばを下処理する。", "2. 調味料を煮立てる。", "3. さばを煮る。"],
        "genre": "japanese"
    },
    {
        "id": 14,
        "name": "八宝菜",
        "category": "main",
        "ingredients": ["白菜", "豚肉", "にんじん", "しいたけ", "たけのこ"], 
        "steps": ["1. 具材を切る。", "2. 肉を炒める。", "3. 野菜を加えて炒める。", "4. 調味料で煮る。"], 
        "genre": "chinese"
    },
    {
        "id": 15,
        "name": "青椒肉絲(チンジャオロース)",
        "category": "main",
        "ingredients": ["牛肉", "ピーマン", "たけのこ"],
        "steps": ["1. 牛肉に下味を付ける。", "2. 野菜を切る。", "3. 炒める。", "4. 調味料を加える。"],
        "genre": "chinese"
    },
    {
        "id": 16,
        "name": "回鍋肉",
        "category": "main",
        "ingredients": ["豚肉", "キャベツ", "ピーマン"],
        "steps": ["1. 具材を切る。", "2. 豚肉を炒める。", "3. 野菜を加えて炒める。", "4. 味噌だれを加える。"],
        "genre": "chinese"
    },
    {
        "id": 17,
        "name": "酢豚",
        "category": "main",
        "ingredients": ["豚肉", "玉ねぎ", "にんじん", "ピーマン"],
        "steps": ["1. 豚肉に下味を付ける。", "2. 揚げる。", "3. 野菜を炒める。", "4. 甘酢あんと合わせる。"],
        "genre": "chinese"
    },
    {
        "id": 18,
        "name": "エビチリ",
        "category": "main",
        "ingredients": ["えび", "長ねぎ"], 
        "steps": ["1. えびの下処理をする。", "2. 炒める。", "3. チリソースを加えて煮からめる。"],
        "genre": "chinese"
    },
    {
        "id": 19,
        "name": "グラタン",
        "category": "main",
        "ingredients": ["鶏肉", "玉ねぎ", "牛乳", "チーズ"], 
        "steps": ["1. 具材を切る。", "2. 炒める。", "3. ホワイトソースと合わせる。", "4. チーズをのせて焼く。"], 
        "genre": "western"
    },
    {
        "id": 20,
        "name": "ラザニア",
        "category": "main",
        "ingredients": ["ひき肉", "チーズ", "トマト"],
        "steps": ["1. ミートソースを作る。", "2. 生地とソースを重ねる。", "3. チーズをのせる。", "4. 焼く。"], 
        "genre": "western"
    },
    {
        "id": 21,
        "name": "ビーフシチュー",
        "category": "main",
        "ingredients": ["牛肉", "玉ねぎ", "にんじん", "じゃがいも"],
        "steps": ["1. 牛肉を焼く。", "2. 野菜を炒める。", "3. 煮込む。"],
        "genre": "western"
    },
    {
        "id": 22,
        "name": "チキンソテー",
        "category": "main",
        "ingredients": ["鶏もも肉"],
        "steps": ["1. 鶏肉に下味を付ける。", "2. 焼く。", "3. ソースをかける。"],
        "genre": "western"
    },
    {
        "id": 23,
        "name": "タンドリーチキン",
        "category": "main",
        "ingredients": ["鶏もも肉", "ヨーグルト"],
        "steps": ["1. 鶏肉を漬け込む。", "2. オーブンまたはフライパンで焼く。"],
        "genre": "エスニック"
    },
    {
        "id": 24,
        "name": "ヤンニョムチキン",
        "category": "main",
        "ingredients": ["鶏もも肉"],
        "steps": ["1. 鶏肉を揚げる。", "2. ヤンニョムソースと和える。"],
        "genre": "korean"
    },
    {
        "id": 25,
        "name": "プルコギ",
        "category": "main",
        "ingredients": ["牛肉", "玉ねぎ", "にんじん"],
        "steps": ["1. 牛肉を下味に漬ける。", "2. 野菜を切る。", "3. 一緒に炒める。"],
        "genre": "korean"
    },
    {
        "id": 26,
        "name": "ゴーヤチャンプルー",
        "category": "main",
        "ingredients": ["ゴーヤ", "豚肉", "豆腐", "卵"],
        "steps": ["1. ゴーヤを下処理する。", "2. 具材を炒める。", "3. 豆腐を加える。", "4. 卵でとじる。"],
        "genre": "japanese"
    },
    {
        "id": 27,
        "name": "筑前煮",
        "category": "main",
        "ingredients": ["鶏肉", "れんこん", "ごぼう", "にんじん", "こんにゃく"],
        "genre": "japanese"
    },
    {
        "id": 28,
        "name": "鶏の甘酢あん",
        "category": "main",
        "ingredients": ["鶏肉", "玉ねぎ", "ピーマン"],
        "genre": "japanese"
    },
    {
        "id": 29,
        "name": "豚の角煮",
        "category": "main",
        "ingredients": ["豚バラ肉", "大根", "しょうが"],
        "genre": "japanese"
    },
    {
        "id": 30,
        "name": "アジフライ",
        "category": "main",
        "ingredients": ["アジ", "パン粉", "卵"],
        "genre": "western"
    },
    {
        "id": 31,
        "name": "カレイの煮付け",
        "category": "main",
        "ingredients": ["カレイ", "しょうが"],
        "genre": "japanese"
    },
    {
        "id": 32,
        "name": "鮭のホイル焼き",
        "category": "main",
        "ingredients": ["鮭", "しめじ", "玉ねぎ", "バター"],
        "genre": "japanese"
    },
    {
        "id": 33,
        "name": "アクアパッツァ",
        "category": "main",
        "ingredients": ["白身魚", "あさり", "ミニトマト"],
        "genre": "western"
    },
    {
        "id": 34,
        "name": "ポークチャップ",
        "category": "main",
        "ingredients": ["豚肉", "玉ねぎ"],
        "genre": "western"
    },
    {
        "id": 35,
        "name": "チキンカツ",
        "category": "main",
        "ingredients": ["鶏むね肉", "パン粉", "卵"],
        "genre": "western"
    },
    {
        "id": 36,
        "name": "クリームシチュー",
        "category": "main",
        "ingredients": ["鶏肉", "じゃがいも", "にんじん", "玉ねぎ", "牛乳"],
        "genre": "western"
    },
    {
        "id": 37,
        "name": "ポークソテー",
        "category": "main",
        "ingredients": ["豚ロース肉"],
        "genre": "western"
    },
    {
        "id": 38,
        "name": "油淋鶏",
        "category": "main",
        "ingredients": ["鶏もも肉", "長ねぎ"],
        "genre": "chinese"
    },
    {
        "id": 39,
        "name": "麻婆豆腐",
        "category": "main",
        "ingredients": ["豆腐", "豚ひき肉", "長ねぎ"],
        "genre": "chinese"
    },
    {
        "id": 40,
        "name": "ニラレバ炒め",
        "category": "main",
        "ingredients": ["豚レバー", "ニラ", "もやし"],
        "genre": "chinese"
    },
    {
        "id": 41,
        "name": "鶏肉とカシューナッツ炒め",
        "category": "main",
        "ingredients": ["鶏肉", "カシューナッツ", "ピーマン", "玉ねぎ"],
        "genre": "chinese"
    },
    {
        "id": 42,
        "name": "チャプチェ",
        "category": "main",
        "ingredients": ["春雨", "牛肉", "にんじん", "ピーマン"],
        "genre": "korean"
    },
    {
        "id": 43,
        "name": "チーズタッカルビ",
        "category": "main",
        "ingredients": ["鶏肉", "キャベツ", "玉ねぎ", "チーズ"],
        "genre": "korean"
    },
    {
        "id": 44,
        "name": "メンチカツ",
        "category": "main",
        "ingredients": ["合いびき肉", "玉ねぎ", "パン粉", "卵"],
        "genre": "western"
    },
    {
        "id": 45,
        "name": "さわらの西京焼き",
        "category": "main",
        "ingredients": ["さわら", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 46,
        "name": "チキンピカタ",
        "category": "main",
        "ingredients": ["鶏むね肉", "卵"],
        "genre": "western"
    },
    {
        "id": 47,
        "name": "メカジキのソテー",
        "category": "main",
        "ingredients": ["メカジキ", "バター"],
        "genre": "western"
    },
    {
        "id": 48,
        "name": "豚しゃぶ",
        "category": "main",
        "ingredients": ["豚肉", "レタス"],
        "genre": "japanese"
    },
    {
        "id": 49,
        "name": "鶏団子の甘酢あん",
        "category": "main",
        "ingredients": ["鶏ひき肉", "玉ねぎ", "ピーマン"],
        "genre": "japanese"
    },
    {
        "id": 50,
        "name": "白身魚のフライ",
        "category": "main",
        "ingredients": ["白身魚", "パン粉", "卵"],
        "genre": "western"
    },
    {
        "id": 101,
        "name": "ポテトサラダ",
        "category": "side",
        "ingredients": ["じゃがいも", "きゅうり", "にんじん", "ハム"],
        "genre": "western"
    },
    {
        "id": 102,
        "name": "ほうれん草のおひたし",
        "category": "side",
        "ingredients": ["ほうれん草", "かつお節"],
        "genre": "japanese"
    },
    {
        "id": 103,
        "name": "きんぴらごぼう",
        "category": "side",
        "ingredients": ["ごぼう", "にんじん"],
        "genre": "japanese"
    },
    {
        "id": 104,
        "name": "マカロニサラダ",
        "category": "side",
        "ingredients": ["マカロニ", "きゅうり", "ハム", "人参"],
        "genre": "western"
    },
    {
        "id": 105,
        "name": "ひじきの煮物",
        "category": "side",
        "ingredients": ["ひじき", "にんじん", "大豆"],
        "genre": "japanese"
    },
    {
        "id": 106,
        "name": "かぼちゃの煮物",
        "category": "side",
        "ingredients": ["かぼちゃ"],
        "genre": "japanese"
    },
    {
        "id": 107,
        "name": "春雨サラダ",
        "category": "side",
        "ingredients": ["春雨", "きゅうり", "ハム"],
        "genre": "chinese"
    },
    {
        "id": 108,
        "name": "小松菜の胡麻和え",
        "category": "side",
        "ingredients": ["小松菜", "ごま"],
        "genre": "japanese"
    },
    {
        "id": 109,
        "name": "ブロッコリーサラダ",
        "category": "side",
        "ingredients": ["ブロッコリー", "ゆで卵"],
        "genre": "western"
    },
    {
        "id": 110,
        "name": "もやしナムル",
        "category": "side",
        "ingredients": ["もやし", "ごま"],
        "genre": "korean"
    },
    {
        "id": 111,
        "name": "切り干し大根の煮物",
        "category": "side",
        "ingredients": ["切り干し大根", "にんじん", "油揚げ"],
        "genre": "japanese"
    },
    {
        "id": 112,
        "name": "なすの煮びたし",
        "category": "side",
        "ingredients": ["なす", "しょうが"],
        "genre": "japanese"
    },
    {
        "id": 113,
        "name": "厚揚げの生姜焼き",
        "category": "side",
        "ingredients": ["厚揚げ", "しょうが", "ねぎ"],
        "genre": "japanese"
    },
    {
        "id": 114,
        "name": "オクラのおかか和え",
        "category": "side",
        "ingredients": ["オクラ", "かつお節"],
        "genre": "japanese"
    },
    {
        "id": 115,
        "name": "冷奴",
        "category": "side",
        "ingredients": ["豆腐", "ねぎ", "しょうが"],
        "genre": "japanese"
    },
    {
        "id": 116,
        "name": "ラタトゥイユ",
        "category": "side",
        "ingredients": ["なす", "ズッキーニ", "パプリカ", "トマト"],
        "genre": "western"
    },
    {
        "id": 117,
        "name": "キャロットラペ",
        "category": "side",
        "ingredients": ["にんじん"],
        "genre": "western"
    },
    {
        "id": 118,
        "name": "カプレーゼ",
        "category": "side",
        "ingredients": ["トマト", "モッツァレラチーズ", "バジル"],
        "genre": "western"
    },
    {
        "id": 119,
        "name": "コールスロー",
        "category": "side",
        "ingredients": ["キャベツ", "にんじん", "コーン"],
        "genre": "western"
    },
    {
        "id": 120,
        "name": "ジャーマンポテト",
        "category": "side",
        "ingredients": ["じゃがいも", "ベーコン", "玉ねぎ"],
        "genre": "western"
    },
    {
        "id": 121,
        "name": "青椒土豆絲",
        "category": "side",
        "ingredients": ["じゃがいも", "ピーマン"],
        "genre": "chinese"
    },
    {
        "id": 122,
        "name": "棒棒鶏",
        "category": "side",
        "ingredients": ["鶏むね肉", "きゅうり"],
        "genre": "chinese"
    },
    {
        "id": 123,
        "name": "chineseくらげ",
        "category": "side",
        "ingredients": ["くらげ", "きゅうり"],
        "genre": "chinese"
    },
    {
        "id": 124,
        "name": "トマトと卵の炒め物",
        "category": "side",
        "ingredients": ["トマト", "卵"],
        "genre": "chinese"
    },
    {
        "id": 125,
        "name": "ニラ玉",
        "category": "side",
        "ingredients": ["ニラ", "卵"],
        "genre": "chinese"
    },
    {
        "id": 126,
        "name": "チョレギサラダ",
        "category": "side",
        "ingredients": ["レタス", "きゅうり", "koreanのり"],
        "genre": "korean"
    },
    {
        "id": 127,
        "name": "ほうれん草ナムル",
        "category": "side",
        "ingredients": ["ほうれん草", "ごま"],
        "genre": "korean"
    },
    {
        "id": 128,
        "name": "キムチ豆腐",
        "category": "side",
        "ingredients": ["豆腐", "キムチ", "ねぎ"],
        "genre": "chinese"
    },
    {
        "id": 129,
        "name": "れんこんサラダ",
        "category": "side",
        "ingredients": ["れんこん", "きゅうり", "にんじん"],
        "genre": "western"
    },
    {
        "id": 130,
        "name": "アボカドとトマトのサラダ",
        "category": "side",
        "ingredients": ["アボカド", "トマト"],
        "genre": "western"
    },
    {
        "id": 131,
        "name": "焼きなす",
        "category": "side",
        "ingredients": ["なす", "しょうが", "かつお節"],
        "genre": "japanese"
    },
    {
        "id": 132,
        "name": "きゅうりの浅漬け",
        "category": "side",
        "ingredients": ["きゅうり"],
        "genre": "japanese"
    },
    {
        "id": 133,
        "name": "白菜の浅漬け",
        "category": "side",
        "ingredients": ["白菜"],
        "genre": "japanese"
    },
    {
        "id": 134,
        "name": "もずく酢",
        "category": "side",
        "ingredients": ["もずく"],
        "genre": "japanese"
    },
    {
        "id": 135,
        "name": "ひやしトマト",
        "category": "side",
        "ingredients": ["トマト"],
        "genre": "western"
    },
    {
        "id": 136,
        "name": "いんげんの胡麻和え",
        "category": "side",
        "ingredients": ["いんげん", "ごま"],
        "genre": "japanese"
    },
    {
        "id": 137,
        "name": "ほうれん草とベーコンのソテー",
        "category": "side",
        "ingredients": ["ほうれん草", "ベーコン"],
        "genre": "western"
    },
    {
        "id": 138,
        "name": "きのこのソテー",
        "category": "side",
        "ingredients": ["しめじ", "しいたけ", "えのき"],
        "genre": "western"
    },
    {
        "id": 139,
        "name": "アスパラベーコン",
        "category": "side",
        "ingredients": ["アスパラガス", "ベーコン"],
        "genre": "western"
    },
    {
        "id": 140,
        "name": "ジャーマンかぼちゃ",
        "category": "side",
        "ingredients": ["かぼちゃ", "ベーコン", "玉ねぎ"],
        "genre": "western"
    },
    {
        "id": 141,
        "name": "カリフラワーサラダ",
        "category": "side",
        "ingredients": ["カリフラワー", "ゆで卵"],
        "genre": "western"
    },
    {
        "id": 142,
        "name": "ツナサラダ",
        "category": "side",
        "ingredients": ["レタス", "きゅうり", "トマト", "ツナ"],
        "genre": "western"
    },
    {
        "id": 143,
        "name": "ブロッコリーとツナのサラダ",
        "category": "side",
        "ingredients": ["ブロッコリー", "ツナ"],
        "genre": "western"
    },
    {
        "id": 144,
        "name": "コーンバター",
        "category": "side",
        "ingredients": ["コーン", "バター"],
        "genre": "western"
    },
    {
        "id": 145,
        "name": "きくらげと卵の炒め物",
        "category": "side",
        "ingredients": ["きくらげ", "卵"],
        "genre": "chinese"
    },
    {
        "id": 146,
        "name": "ザーサイ",
        "category": "side",
        "ingredients": ["ザーサイ"],
        "genre": "chinese"
    },
    {
        "id": 147,
        "name": "きゅうりのchinese和え",
        "category": "side",
        "ingredients": ["きゅうり", "ごま"],
        "genre": "chinese"
    },
    {
        "id": 148,
        "name": "白菜ナムル",
        "category": "side",
        "ingredients": ["白菜", "ごま"],
        "genre": "korean"
    },
    {
        "id": 149,
        "name": "豆苗炒め",
        "category": "side",
        "ingredients": ["豆苗", "にんにく"],
        "genre": "chinese"
    },
    {
        "id": 150,
        "name": "korean風きゅうりサラダ",
        "category": "side",
        "ingredients": ["きゅうり", "キムチ", "ごま"],
        "genre": "korean"
    }, 
    {
        "id": 201,
        "name": "味噌汁",
        "category": "soup",
        "ingredients": ["豆腐", "わかめ", "長ねぎ", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 202,
        "name": "豚汁",
        "category": "soup",
        "ingredients": ["豚肉", "大根", "にんじん", "ごぼう", "こんにゃく", "長ねぎ", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 203,
        "name": "けんちん汁",
        "category": "soup",
        "ingredients": ["大根", "にんじん", "ごぼう", "豆腐", "こんにゃく", "長ねぎ"],
        "genre": "japanese"
    },
    {
        "id": 204,
        "name": "かきたま汁",
        "category": "soup",
        "ingredients": ["卵", "長ねぎ", "片栗粉"],
        "genre": "japanese"
    },
    {
        "id": 205,
        "name": "わかめスープ",
        "category": "soup",
        "ingredients": ["わかめ", "長ねぎ", "ごま"],
        "genre": "chinese"
    },
    {
        "id": 206,
        "name": "コーンスープ",
        "category": "soup",
        "ingredients": ["コーン", "牛乳", "玉ねぎ", "バター"],
        "genre": "western"
    },
    {
        "id": 207,
        "name": "ミネストローネ",
        "category": "soup",
        "ingredients": ["トマト", "玉ねぎ", "にんじん", "じゃがいも", "キャベツ"],
        "genre": "western"
    },
    {
        "id": 208,
        "name": "オニオンスープ",
        "category": "soup",
        "ingredients": ["玉ねぎ", "バター", "コンソメ"],
        "genre": "western"
    },
    {
        "id": 209,
        "name": "chineseスープ",
        "category": "soup",
        "ingredients": ["卵", "長ねぎ", "ごま", "鶏がらスープの素"],
        "genre": "chinese"
    },
    {
        "id": 210,
        "name": "クラムチャウダー",
        "category": "soup",
        "ingredients": ["あさり", "じゃがいも", "玉ねぎ", "にんじん", "牛乳", "バター"],
        "genre": "western"
    },
    {
        "id": 211,
        "name": "豆腐と油揚げの味噌汁",
        "category": "soup",
        "ingredients": ["豆腐", "油揚げ", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 212,
        "name": "なめこの味噌汁",
        "category": "soup",
        "ingredients": ["なめこ", "豆腐", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 213,
        "name": "しじみの味噌汁",
        "category": "soup",
        "ingredients": ["しじみ", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 214,
        "name": "あさりの味噌汁",
        "category": "soup",
        "ingredients": ["あさり", "長ねぎ", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 215,
        "name": "キャベツの味噌汁",
        "category": "soup",
        "ingredients": ["キャベツ", "油揚げ", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 216,
        "name": "じゃがいもの味噌汁",
        "category": "soup",
        "ingredients": ["じゃがいも", "玉ねぎ", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 217,
        "name": "ほうれん草の味噌汁",
        "category": "soup",
        "ingredients": ["ほうれん草", "豆腐", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 218,
        "name": "白菜の味噌汁",
        "category": "soup",
        "ingredients": ["白菜", "油揚げ", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 219,
        "name": "大根の味噌汁",
        "category": "soup",
        "ingredients": ["大根", "油揚げ", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 220,
        "name": "小松菜の味噌汁",
        "category": "soup",
        "ingredients": ["小松菜", "豆腐", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 221,
        "name": "なすの味噌汁",
        "category": "soup",
        "ingredients": ["なす", "油揚げ", "味噌"],
        "genre": "japanese"
    },
    {
        "id": 222,
        "name": "きのこスープ",
        "category": "soup",
        "ingredients": ["しめじ", "えのき", "しいたけ", "コンソメ"],
        "genre": "western"
    },
    {
        "id": 223,
        "name": "野菜スープ",
        "category": "soup",
        "ingredients": ["キャベツ", "玉ねぎ", "にんじん", "じゃがいも", "コンソメ"],
        "genre": "western"
    },
    {
        "id": 224,
        "name": "白菜とベーコンのスープ",
        "category": "soup",
        "ingredients": ["白菜", "ベーコン", "コンソメ"],
        "genre": "western"
    },
    {
        "id": 225,
        "name": "キャベツとウインナーのスープ",
        "category": "soup",
        "ingredients": ["キャベツ", "ウインナー", "コンソメ"],
        "genre": "western"
    },
    {
        "id": 226,
        "name": "トマトスープ",
        "category": "soup",
        "ingredients": ["トマト", "玉ねぎ", "コンソメ"],
        "genre": "western"
    },
    {
        "id": 227,
        "name": "かぼちゃスープ",
        "category": "soup",
        "ingredients": ["かぼちゃ", "牛乳", "バター"],
        "genre": "western"
    },
    {
        "id": 228,
        "name": "じゃがいものポタージュ",
        "category": "soup",
        "ingredients": ["じゃがいも", "玉ねぎ", "牛乳", "バター"],
        "genre": "western"
    },
    {
        "id": 229,
        "name": "にんじんポタージュ",
        "category": "soup",
        "ingredients": ["にんじん", "玉ねぎ", "牛乳", "バター"],
        "genre": "western"
    },
    {
        "id": 230,
        "name": "ブロッコリーポタージュ",
        "category": "soup",
        "ingredients": ["ブロッコリー", "玉ねぎ", "牛乳"],
        "genre": "western"
    },
    {
        "id": 231,
        "name": "かぼちゃとベーコンのスープ",
        "category": "soup",
        "ingredients": ["かぼちゃ", "ベーコン", "牛乳"],
        "genre": "western"
    },
    {
        "id": 232,
        "name": "卵スープ",
        "category": "soup",
        "ingredients": ["卵", "鶏がらスープの素", "長ねぎ"],
        "genre": "chinese"
    },
    {
        "id": 233,
        "name": "もやしスープ",
        "category": "soup",
        "ingredients": ["もやし", "長ねぎ", "鶏がらスープの素"],
        "genre": "chinese"
    },
    {
        "id": 234,
        "name": "豆腐とニラのスープ",
        "category": "soup",
        "ingredients": ["豆腐", "ニラ", "鶏がらスープの素"],
        "genre": "chinese"
    },
    {
        "id": 235,
        "name": "トマトと卵のchineseスープ",
        "category": "soup",
        "ingredients": ["トマト", "卵", "鶏がらスープの素"],
        "genre": "chinese"
    },
    {
        "id": 236,
        "name": "春雨スープ",
        "category": "soup",
        "ingredients": ["春雨", "長ねぎ", "鶏がらスープの素"],
        "genre": "chinese"
    },
    {
        "id": 237,
        "name": "チンゲン菜のchineseスープ",
        "category": "soup",
        "ingredients": ["チンゲン菜", "卵", "鶏がらスープの素"],
        "genre": "chinese"
    },
    {
        "id": 238,
        "name": "わかめと豆腐のchineseスープ",
        "category": "soup",
        "ingredients": ["わかめ", "豆腐", "鶏がらスープの素"],
        "genre": "chinese"
    },
    {
        "id": 239,
        "name": "酸辣湯",
        "category": "soup",
        "ingredients": ["豆腐", "卵", "しいたけ", "たけのこ", "片栗粉"],
        "genre": "chinese"
    },
    {
        "id": 240,
        "name": "ユッケジャンスープ",
        "category": "soup",
        "ingredients": ["牛肉", "もやし", "長ねぎ", "ニラ"],
        "genre": "korean"
    },
    {
        "id": 241,
        "name": "トックスープ",
        "category": "soup",
        "ingredients": ["トック", "卵", "長ねぎ"],
        "genre": "korean"
    },
    {
        "id": 242,
        "name": "サムゲタンスープ",
        "category": "soup",
        "ingredients": ["鶏肉", "長ねぎ", "しょうが"],
        "genre": "korean"
    },
    {
        "id": 243,
        "name": "ビシソワーズ",
        "category": "soup",
        "ingredients": ["じゃがいも", "玉ねぎ", "牛乳"],
        "genre": "western"
    },
    {
        "id": 244,
        "name": "パンプキンポタージュ",
        "category": "soup",
        "ingredients": ["かぼちゃ", "牛乳", "バター"],
        "genre": "western"
    },
    {
        "id": 245,
        "name": "マッシュルームスープ",
        "category": "soup",
        "ingredients": ["マッシュルーム", "玉ねぎ", "牛乳"],
        "genre": "western"
    },
    {
        "id": 246,
        "name": "ブロッコリーとベーコンのスープ",
        "category": "soup",
        "ingredients": ["ブロッコリー", "ベーコン", "コンソメ"],
        "genre": "western"
    },
    {
        "id": 247,
        "name": "レタススープ",
        "category": "soup",
        "ingredients": ["レタス", "卵", "コンソメ"],
        "genre": "western"
    },
    {
        "id": 248,
        "name": "ズッキーニのスープ",
        "category": "soup",
        "ingredients": ["ズッキーニ", "玉ねぎ", "コンソメ"],
        "genre": "western"
    },
    {
        "id": 249,
        "name": "トマトとキャベツのスープ",
        "category": "soup",
        "ingredients": ["トマト", "キャベツ", "玉ねぎ", "コンソメ"],
        "genre": "western"
    },
    {
        "id": 250,
        "name": "ほうれん草とベーコンのスープ",
        "category": "soup",
        "ingredients": ["ほうれん草", "ベーコン", "コンソメ"],
        "genre": "western"
    }
]